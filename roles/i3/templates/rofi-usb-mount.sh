...
usbcheck(){ \
    mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -E 'sd[c-z]' | awk '$2=="part"&&$4!=""{printf "%s (%s)\t  ",$1,$3}')"
...
umountall(){ \
    for chosen in $(echo $(lsblk -rpo "name,type,size,mountpoint" | grep -E 'sd[c-z]' | awk '$2=="part"&&$4!=""{printf "%s\n",$1}')); do
...
usbdrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -E 'sd[c-z]' | awk '$2=="part"&&$4==""{printf "%s (%s)\n",$1,$3}')"
mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | grep -E 'sd[c-z]' | awk '$2=="part"&&$4!=""{printf "%s (%s)\n",$1,$3}')"
